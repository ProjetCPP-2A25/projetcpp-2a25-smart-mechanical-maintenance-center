...
void MainWindow::on_pushButton_ajouter_clicked() {
    ...
    //int dl = ui->lineEdit_dl->text().toInt(); // Get dl as a QString
    ...
    // Préparer la requête pour vérifier si l'ID existe
    checkQuery.prepare("SELECT COUNT(*) FROM FOURNISSEUR WHERE ID = :id");

    // Lier la valeur de l'ID à la requête
    ...
    if (checkQuery.exec()) {
        if (checkQuery.next()) {
            ...
            if (count > 0) {
                qDebug() << "L'ID" << id << "existe dans la table FOURNISSEUR.";
            } else {
                qDebug() << "L'ID" << id << "n'existe pas dans la table FOURNISSEUR.";
            }
        }
    } else {
